    ...
    <div class="page-header" id="banner">
        <div class="row">
            ...
            <div class="col-lg-8 col-md-7 col-sm-6">
                <a onclick="mage.setUrl('<?php echo $this->getUrl()->getUrl('form', 'admin\customer', null, true); ?>').setMethod('post').load();" href="javascript:void(0);" class="btn btn-primary" name="update">Add Customer
                                ...
                                <?php foreach ($data->getData() as $value) : ?>
                                    <tr>
                                        <?php foreach ($this->getColumns() as $column) : ?>
                                            <th>
                                                <?php if (array_key_exists('method', $column)) : ?>
                                                    <?php
                                                    $method = $column['method'];
                                                    print_r($this->$method($value->customerId));
                                                    //echo $this->$column['method']($value->customerId); 
                                                    ?>
                                                <?php else : ?>
                                                    <?php echo $this->getFieldValue($value, $column['field']); ?>
                                                <?php endif; ?>
                                            </th>
                                        <?php endforeach; ?>
